...
export async function openApplicationPanel(applicationId, onSaved) {
  ...
  const linkedIds = linkedStories.map(s => String(s.story_id))
    ...
    <div class="panel-grid">
      ...
      <div class="panel-field">
        ...
        <select id="panel-stability">
          ${STABILITY.map(s => `
            <option value="${s}" ${s === application.stability_check ? 'selected' : ''}>
              ${s}
            </option>
          `).join('')}
        </select>
      ...
      <div class="panel-field panel-field--full">
        ...
        <input id="panel-job-link" value="${application.job_link || ''}" placeholder="https://..." />
      </div>
      ...
      <div class="panel-field panel-field--full">
        <label>Notes</label>
        <textarea id="panel-notes" rows="5">${application.notes || ''}</textarea>
      </div>
  ...
  document.querySelector('#save-panel').addEventListener('click', async () => {
    ...
    await updateApplication({
      ...
      jira_id: document.querySelector('#panel-jira-id').value,
      prep_page_url: document.querySelector('#panel-prep-page-url').value,
      stability_check: document.querySelector('#panel-stability').value,
      next_action: document.querySelector('#panel-next-action').value,
      notes: document.querySelector('#panel-notes').value
    })

    await saveLinkedStories(application.id, selectedStories)
