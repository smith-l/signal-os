...
export async function onRequestPut(context) {
  const body = await context.request.json()
  await context.env.signal_os_db
    .prepare(`UPDATE applications SET company=?, role_title=?, status=?, next_action=?, recruiter=?, salary=?, job_link=?, location=?, jira_id=?, prep_page_url=?, stability_check=?, notes=?, applied_date=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`)
    .bind(body.company, body.role_title, body.status, body.next_action || '', body.recruiter || '', body.salary || '', body.job_link || '', body.location || '', body.jira_id || '', body.prep_page_url || '', body.stability_check || '', body.notes || '', body.applied_date || '', body.id)
    .run()
  return Response.json({ success: true })
}
